...
from seedingBcell import residue_probs
from seedingBcell import residue_escape
    ...
    for t in range(nb_trial):
        ...
        for i in range(len(exit_cells)):
            for b in exit_cells[i]:
                if b.nb > 50:
                    writestring = ('%d,'*5 + '%lf,'*(length+3))[:-1]
                    bres = tuple([b.res[i] for i in range(length)])
                    inittuple = tuple([t, b.cycle_number, b.nb, b.generation, b.nb_CDR_mut, b.E, b.Ec, b.breadth])
                    muttuple = tuple([b.history['mut_res_id'], b.history['delta_e']])
                    
                    fend.write(writestring % (inittuple+bres))
                    fend.write('\n')
                    
                    fendmutstring = ('%d,'*5 + '%lf,'*3 + '%s,'*len(muttuple) + '%lf,'*length)[:-1]
                    fendmut.write(fendmutstring % (inittuple+muttuple+bres))
                    fendmut.write('\n')
        ...
        for i in range(len(recycled_cells)):    
            meanE, meanEc, meanBreadth, meanCDRMutations, count_clones = 0, 0, 0, 0, 0
            if nb_recycled[i] > 0:
            ...
            writestring = '%d,'*4 + '%lf,'*4
            inittuple = (t, cycle, nb_recycled[i], nb_exit[i], meanE, meanEc, meanBreadth, meanCDRMutations)
            ftot.write(writestring % inittuple)
        ftot.flush()
    ...
    # End and output total time
        ...
        if len(exit_cells[-1]) > 0:
            for j in range(len(exit_cells[-1])):
                b = exit_cells[-1][j]
                if b.nb > 50:
                    with open('output/history_largest_clone.csv', 'w') as h:
                        w = csv.writer(h)
                        w.writerows(b.history.items())
             
                    for i in range(len(b.history['E'])):
                        writestring = '%d,'*5 + '%lf,'*(length+3) + '\n'
                        bhis = tuple([b.history['res'][i][j] for j in range(length)])
                        inittuple = tuple([t, b.history['cycle_number'][i],
                                          i, b.history['generation'][i],
                                          b.history['nb_CDR_mut'][i],
                                          b.history['E'][i],
                                          b.history['delta_res'][i],
                                          b.history['mut_res_id'][i]])
                        fbig.write(writestring % (inittuple+bhis))
                        
                    bres = tuple([b.res[i] for i in range(length)])
                    inittuple = tuple([t, b.cycle_number, len(b.history['E']),
                                       b.generation, b.nb_CDR_mut, b.E,
                                       b.delta_res, b.mut_res_id])
                    fbig.write(writestring % (inittuple+bres))
            fbig.flush()
